...
decays. We offer UMA-Inverse as a compact, MSA-free baseline for ligand-conditioned inverse
folding---trailing LigandMPNN in accuracy---and a characterization of how a dense all-pairs encoder
...
\begin{enumerate}
  ...
  \item Nucleic-acid routing (DNA/RNA chains into the ligand pool) enabling the first nucleotide-split
        ...
        ligand-agnostic baseline; we trace this to how the dense encoder admits ligand atoms, not to
        atom conditioning per se, and argue it motivates a hybrid design (\S\ref{sec:discussion}).
...
UMA-Inverse iterations could not address. We flag upfront that ligand atoms enter as a single global
set capped at 50 (the nearest to the protein centroid), which covers a compact ligand but only a small
fragment of a nucleic acid; \S\ref{sec:results_iface} shows this is insufficient---indeed
counterproductive---for nucleotide partners, with the cause and a fix discussed in \S\ref{sec:discussion}.
...
Notably, an atom-context representation is not inherently at fault---LigandMPNN also conditions on
nucleic-acid atoms and outperforms ProteinMPNN here. We trace the failure instead to how UMA-Inverse's
dense encoder admits ligand atoms, and discuss it---with a hybrid remedy---in \S\ref{sec:discussion}.
...
nucleic-acid routing is an outright negative result (below the ligand-agnostic baseline) for an
...
even the ligand-agnostic ProteinMPNN ($47.1\%$), so its nucleic-acid conditioning is net-negative.
Crucially, an atom-context representation is not inherently at fault---LigandMPNN also conditions on
nucleic-acid atoms and outperforms ProteinMPNN here; the difference is architectural. UMA-Inverse admits
...
each residue its own local atom neighborhood, which scales to a macromolecule. This exposes an
...
budget. We did not run the controlled experiment to confirm this is the dominant cause of the nucleotide
failure, and leave the diagnosis and the hybrid remedy to future work. For the protein-ligand splits we
similarly lack a controlled ablation isolating the architectural source of the residual gaps, and do
not attribute them to a specific cause. We also
...
nucleic-acid routing is a negative result (below the ligand-agnostic baseline) that we trace to its
dense encoder's global ligand-atom budget and argue points toward a hybrid design (\S\ref{sec:discussion}).
Its contribution is to show that a compact, MSA-free,
