...
than we reproduce under this protocol). In a pocket-fixed setting---holding the native
...
evaluation on the nucleotide test split (\S\ref{sec:results_iface}), which prior UMA-Inverse iterations
could not address.
...
is, to our knowledge, the first such comparison for this model line, enabled by routing nucleic-acid
chains into the ligand context; at $35.3\%$ it is the largest remaining gap and trails both baselines,
indicating that ingesting nucleic-acid context is necessary but not yet sufficient for nucleotide
design---a clear target for future work.
...
ligand-conditioned inverse folding: it trains on commodity GPUs, reaches within $5$--$7$\,pp of
LigandMPNN's interface recovery, supports nucleic-acid partners, and produces confidently folded,
ligand-binding-competent sequences. It does not, however, match LigandMPNN, which is more accurate on
interface recovery, on pocket-fixed distal recovery, and on cofolded ligand pose. We therefore present
...
Against our matched-protocol LigandMPNN, the largest residual gap is on nucleotide
($35.3\%$ vs.\ $53.3\%$); the metal and small-molecule gaps are smaller under this controlled
comparison ($55.1\%$ vs.\ $64.4\%$ and $56.1\%$ vs.\ $59.8\%$) than LigandMPNN's published numbers
imply. We do not have a controlled ablation isolating the architectural source of these gaps, and so
do not attribute them to a specific cause. We also
...
single-temperature snapshot reported here. Other extensions include flow-matching~\citep{lipman2022flow}
in place of cross-entropy, full RDKit-derived ligand chemistry and bond topology (scaffolded but
disabled in this model), and adding triangle self-attention to the pair encoder.
...
interface recovery across small-molecule, metal, and nucleotide partners, with confidently folded,
ligand-binding-competent cofolds. It does not surpass LigandMPNN on recovery or cofolded pose; its
contribution is to show that a compact, MSA-free, distogram-supervised dense pair encoder is a viable
alternative architecture, and to characterize its distinctive propagation of ligand information to
residues far beyond the binding pocket.
