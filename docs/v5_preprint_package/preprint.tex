...
We did not run the controlled experiment to confirm this is the dominant cause of the nucleotide
failure, and leave that confirmation to future work. For the protein-ligand splits we
similarly lack a controlled ablation isolating the architectural source of the residual gaps, and do
not attribute them to a specific cause.
...
single-temperature snapshot reported here. Realizing the \emph{hybrid encoder} argued for in
\S\ref{sec:discussion}, a dense residue tensor for geometry plus a per-residue local atom-context
