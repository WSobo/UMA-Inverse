...
on full ligand geometry. We present \textbf{UMA-Inverse} (Unified Molecule-Aware Inverse folding),
...
readout of the pair tensor. The model is compact ($\sim$3.3\,M parameters) and trains on a single
commodity GPU. On the LigandMPNN test splits it reaches 56.1\%/55.1\%/35.3\% interface recovery
...
$63.3$/$77.5$/$50.5$). On small molecule and metal the ligand conditioning helps---UMA-Inverse exceeds
the ligand-agnostic ProteinMPNN---but on nucleotide it falls \emph{below} ProteinMPNN, a negative
result: a nucleic acid is a macromolecule, ill-suited to a fixed-size ligand-atom context. In a
...
\begin{figure}[t]
  ...
  \caption{\textbf{UMA-Inverse architecture.} Backbone and ligand/nucleic-acid atoms are featurized
    ...
    tensor $Z$ over all residue--residue and residue--ligand atom pairs. An auxiliary distogram head
    supervises $Z$ during training, and an autoregressive decoder reads ligand context from $Z$ through
    a learned, position-specific attention to emit the sequence. Each PairMixer block (bottom) applies
    incoming and outgoing triangle multiplication and a pair-transition MLP with residual connections;
    triangle self-attention is omitted.}
