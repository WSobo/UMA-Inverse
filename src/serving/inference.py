...
def _default_config_path() -> Path:
    """Path to the Hydra config that defines the model architecture (+ data
    params). Must match the served checkpoint's training arch. Override with
    UMA_CONFIG_PATH; defaults to the repo's configs/config.yaml (now the v5 arch)."""
...
class InferenceEngine:
    ...
    def __init__(
        ...
        *,
        ...
        # from_checkpoint builds the model architecture from the Hydra config
        # (cfg.model + cfg.data flags), NOT from the checkpoint, then loads the
        # weights. The config MUST match the checkpoint's training arch, or the
        # state_dict load is partial and the served model is silently wrong.
...
def get_engine() -> InferenceEngine:
    ...
    if _engine is None:
        with _engine_lock:
            if _engine is None:
                # UMA_CKPT pins a specific checkpoint (e.g. the local v5 weights
                # that match this repo's model code). Unset → resolve_checkpoint
                # (None) auto-fetches the canonical HF weights.
                _engine = InferenceEngine(checkpoint=os.environ.get("UMA_CKPT") or None)
    return _engine
